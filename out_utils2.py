...
def activation_outlier_hook(module, input):
    ...
    outlier_idx_z = find_outlier_dims(
        merged,
        reduction_dim=0,
        zscore=tracer.zscore,   # from args
        topk=None,
    )

    # 2) define outlier condition. magnitude > out_mag, test: outlier
    dims = (merged.abs() > tracer.out_mag).sum(dim=0)   # shape (H,)
...
class OutlierTracer:
    ...
    def initialize(self, model, zscore=3.0, out_mag=5.0, split_exponent=2, quant_method='muxq', quant_mode='per-tensor', act_bits=8, weight_bits=8):
        self.last_w = None
...
@torch.no_grad()
def per_row_absmax_scale(x2d, qmax=None, bits=8, eps=1e-12):
...
@torch.no_grad()
def quantize_int8_per_row(x2d, bits=8, eps=1e-12):
    # x2d: (N, K)
    qmax = 2**(bits-1) - 1
    s = per_row_absmax_scale(x2d, qmax=qmax, bits=bits, eps=eps)    # (N,)
...
@torch.no_grad()
def per_channel_absmax_scale(x2d, qmax=None, bits=8, eps=1e-12):
    """
    x2d: (N, K)
...
@torch.no_grad()
def quantize_int8_per_channel(x2d, bits=8, eps=1e-12):
    """
    x2d: (N, K)
    returns:
        xq: (N, K) int8
    ...
    """
    qmax = 2**(bits-1) - 1
    s = per_channel_absmax_scale(x2d, qmax=qmax, bits=bits, eps=eps)   # (K,)
    xq = torch.round(x2d / s.view(1, -1)).clamp_(-qmax-1, qmax).to(torch.int8)
    return xq, s
...
class MixedInputInt8Conv1D(nn.Module):
    """
    Mixed-precision quantization Conv1D layer.
    Compatible with transformers.Conv1D weight format: (nf, nx) = (out_features, in_features)
    """
    def __init__(self, nf, nx, selected_cols=None, act_bits_non=8, w_bits=8):
        ...
        self.register_buffer("non_idx", None, persistent=False)

        # FP weights / bias - same shape as Conv1D: weight(nf, nx), bias(nf)
        self.weight = nn.Parameter(torch.empty(nf, nx))
        self.bias   = nn.Parameter(torch.zeros(nf))
        nn.init.normal_(self.weight, std=0.02)

        # Quantized weight buffers
        self.register_buffer("W_int8", None, persistent=False)   # (nf, nx), int8
        self.register_buffer("W_scale", None, persistent=False)  # (nf,), float



    def _update_index_buffers(self, mask: torch.Tensor, idx: torch.Tensor):
        ...
        self.sel_idx = idx
        self.non_idx = torch.arange(self.nx, device=mask.device, dtype=torch.long)[non_mask]
    ...
    def _set_selected_cols_from_idx(self, idx: torch.Tensor):
        ...
        if idx is None or idx.numel() == 0:
            ...
            idx = torch.empty(0, dtype=torch.long, device=self.weight.device)
        else:
        ...
        self._update_index_buffers(mask, idx)
    ...
    @torch.no_grad()
    def prepare(self, weight_bits=8): # Weight quantization 
        # Per-output-channel quantization
        # Weight shape: (nf, nx) = (out_features, in_features)
        # Each output channel (row) gets its own scale
        qmax = 2**(weight_bits-1) - 1  # e.g., 127 for int8
        m = self.weight.abs().amax(dim=1, keepdim=True).clamp_min(1e-12)  # (nf, 1) - max per output channel
        s = (m / qmax).squeeze(1)                                         # (nf,) - scale per output channel
        wq = torch.round(self.weight / s.view(-1, 1)).clamp_(-qmax-1, qmax).to(torch.int8)  # (nf, nx)
        # Transpose for matmul: (nx, nf) for compatibility with forward pass
        self.W_int8 = wq.t()  # (nx, nf) for forward matmul
        self.W_scale = s
        self.W_fp = self.weight

    @staticmethod
    @torch.no_grad()
    def split_outlier_columns_fp(x2d, out_idx, split_exponent=2):
        """
        x2d   : (N, K) float
        out_idx: (Ks,) long, outlier column indices in [0, K)
        split_exponent: exponent for splitting (alpha, beta)
                       * 0.5 => exponent=1, * 0.25 => exponent=2, * 0.125 => exponent=3
        return:
            x_main: (N, K)   # outlier shifted by 0.5^split_exponent
            x_out : (N, Ks)  # outlier shifted by 0.5^split_exponent
        """
        ...
        split_scale = 0.5 ** split_exponent

        x_out               =   x_main[:, out_idx] * split_scale  # * 0.5 => exponent=1, * 0.25=> exponent=2, * 0.125 => exponent=3
        x_main[:, out_idx]  =   x_main[:, out_idx] * split_scale  # * 0.5 => exponent=1, *0.25 => exponent=2, * 0.125 => exponent = 3     
    ...
    def forward(self, x):
        ...
        # Get parameters from tracer FIRST (before using weight_bits)
        try:
            tracer = OutlierTracer.get_instance()
            if tracer.is_initialized():
                quant_method = tracer.quant_method
                quant_mode = tracer.quant_mode
                split_exp = tracer.split_exponent
                act_bits = tracer.act_bits
                weight_bits = tracer.weight_bits
            else:
                # Tracer not initialized - use naive method
                quant_method = 'naive'
        ...
        if self.W_int8 is None or self.W_scale is None:
            self.prepare(weight_bits=weight_bits)
        B, T, K = x.shape
        assert K == self.nx
        N = B * T
        ...
        if quant_method == 'muxq':
            ######################### ##################
            ####### Per-Vector Mode (Per-Row)  ########
            ######################### ##################
            if quant_mode == 'per-vector':
                # ===== 1) outlier column decomposition =====
                if self.sel_mask.any():
                    out_idx = self.sel_idx   # (Ks,)
                    x_main, x_out = self.split_outlier_columns_fp(x2d, out_idx, split_exponent=split_exp)
                else:
                    x_main = x2d
                    x_out = None
                ...
                if x_main is not None and x_main.numel() > 0:
                    x_main_q8, s_main_row = quantize_int8_per_row(x_main, bits=act_bits)   # (N,K), (N,)
                ...
                if x_out is not None and x_out.numel() > 0:
                    x_out_q8, s_out_row = quantize_int8_per_row(x_out, bits=act_bits)      # (N,Ks), (N,)
            ...
            ######################### ##################
                ...
                W_q8 = torch.round(self.W_fp / s_w).clamp_(-qmax_w-1, qmax_w).to(torch.int8)   # (nf, nx)
                W_q8 = W_q8.t()  # Transpose to (nx, nf) for matmul
            ...
            # MUXQ merge
            merge_scale = (2 ** split_exp) - 1
            y = y_main + y_out * merge_scale + self.bias.view(1, self.nf)
        ...
        ######################### ##################
            ...
            if quant_mode == 'per-vector':
                ...
                if x_main is not None and x_main.numel() > 0:
                    # (N, K), (N,)
                    x_main_q8, s_main_row = quantize_int8_per_row(x_main, bits=act_bits)
            ...
            # LLM.int8() merge (no scaling)
            y = y_main + y_out + self.bias.view(1, self.nf)
        
        ######################### ##################
        ######################### ##################
        ####### Naive Method (No Outlier Handling) ##
        ######################### ##################
        ######################### ##################
        
        elif quant_method == 'naive':
            # No quantization - pure FP32 computation, same as original Conv1D
            # x2d: (N, nx), W_fp: (nf, nx) -> x2d @ W_fp.t() -> (N, nf)
            y = torch.matmul(x2d, self.W_fp.t()) + self.bias.view(1, self.nf)
