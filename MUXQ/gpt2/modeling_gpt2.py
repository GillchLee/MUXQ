...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if self.is_cross_attention:
            self.c_attn = MixedInputInt8Conv1D(2 * self.embed_dim, self.embed_dim)
            self.q_attn = MixedInputInt8Conv1D(self.embed_dim, self.embed_dim)
        else:
            self.c_attn = MixedInputInt8Conv1D(3 * self.embed_dim, self.embed_dim)
        self.c_proj = MixedInputInt8Conv1D(self.embed_dim, self.embed_dim)
...
class GPT2MLP(nn.Module):
    def __init__(self, intermediate_size, config):
        ...
        self.c_fc = MixedInputInt8Conv1D(intermediate_size, embed_dim)
        self.c_proj = MixedInputInt8Conv1D(embed_dim, intermediate_size)
